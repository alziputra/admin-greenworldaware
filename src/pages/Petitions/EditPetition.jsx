...
const EditPetition = () => {
  ...
  const { editPetition, petitionByID, loading } = usePetition();

  useEffect(() => {
    // Check if newsById is available before setting the title
    if (petitionByID?.title || petitionByID?.description || petitionByID?.image) {
      setTitle(petitionByID.title);
      setDescription(petitionByID.description);
      setFileUrl(petitionByID.image);
    }
  }, [petitionByID]);
  ...
  const handleSubmit = (e) => {
    ...
    dispatch({
      type: 'EDIT_PETITION',
      payload: newPetitionData,
    });
    editPetition(newPetitionData);
  };
        ...
        <form onSubmit={handleSubmit}>
          <div className="items-stretch self-stretch flex flex-col">
            <div className="flex gap-5 mt-2 px-5 max-md:max-w-full max-md:flex-wrap">
              <div className="text-black text-3xl font-medium  shrink basis-auto">Petition</div>
